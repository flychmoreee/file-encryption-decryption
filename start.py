...
def encrypt_files_in_directory(directory, key, file_extensions):
    if os.path.isfile(directory):
        encrypt_file(key, directory)
        print(f"{directory} berhasil dienkripsi.")
    else:
...
def decrypt_files_in_directory(directory, key, file_extensions):
    if os.path.isfile(directory):
        decrypt_file(key, directory)
        print(f"{directory} berhasil didekripsi.")
    else:
        for filename in os.listdir(directory):
            filepath = os.path.join(directory, filename)
            if os.path.isfile(filepath) and any(filename.lower().endswith(ext) for ext in file_extensions):
...
def choose_action():
    while True:
        print("\nPilih aksi:")
        print("1. Enkripsi file dalam direktori atau file tunggal")
        print("2. Dekripsi file dalam direktori atau file tunggal")
        print("0. Keluar")
        
        action = input("Masukkan pilihan Anda: ")
        
        if action == '1':
            path = input("Masukkan direktori atau file yang akan dienkripsi: ")
            key = generate_key()
            file_extensions = choose_format()
            encrypt_files_in_directory(path, key, file_extensions)
            print(f"Kunci enkripsi Anda: {key.decode()}")  # Menampilkan kunci enkripsi, bisa disimpan secara aman
        elif action == '2':
            path = input("Masukkan direktori atau file yang terenkripsi: ")
            key = input("Masukkan kunci enkripsi: ").encode()
            file_extensions = choose_format()
            decrypt_files_in_directory(path, key, file_extensions)
        elif action == '0':
            print("Terima kasih!")
            break
        else:
